...
@Composable
fun Waste(navController: NavController?) {
    ...
    val newIssuesCount = WasteData.count { (it.timestamp?.time ?: 0L) > lastCheckTime }

    Scaffold(
        topBar = { AdminHeader(title = "Waste Reports", navController = navController, onBackClick = { navController?.popBackStack() }) }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
